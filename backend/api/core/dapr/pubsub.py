...
Provides event-driven communication abstraction using DAPR Pub/Sub API.
Enables loose coupling between bounded contexts through message passing.
"""
...
class DaprPubSubClient:
    ...
    This client enables modules to communicate through events,
    ...
    PUBSUB_NAME = "pubsub"  # Default pub/sub component name
    ...
    async def __aenter__(self) -> "DaprPubSubClient":
        return self
    ...
    async def publish_bulk(
        ...
        Args:
            topic: Topic to publish to
            events: List of event entries with entryId and event data
            pubsub_name: Optional pub/sub component name
        ...
        response = await self._client.post(url, json=events)
        response.raise_for_status()
    ...
    def subscribe(
        ...
        This method registers a subscription that should be exposed
        via a route handler in your application.
