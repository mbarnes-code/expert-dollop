/**
 * Prerequisites processor for Commander Spellbook combos.
 * Generates human-readable prerequisite descriptions from combo data.
 */

import { CardInVariant, TemplateInVariant, Variant } from '@space-cow-media/spellbook-client';
import { ComboPrerequisites, getName, getNameBeforeComma, getTypes } from './spellbook-types';
...
/**
 * Join strings with commas and a final "and" or "or".
 */
function comaAndOrJoin(input: string[], joiner = 'and'): string {
  if (input.length === 0) {
    return '';
  }
  if (input.length === 1) {
...
/**
 * Get the state description for a card across different zones.
 */
function getCardState(card: CardInVariant | TemplateInVariant): string {
...
/**
 * Get the list of prerequisites for a combo variant.
 * @param variant - The combo variant
 * @returns Array of prerequisite descriptions with zone information
 */
export function getPrerequisiteList(variant: Variant): ComboPrerequisites[] {
  ...
  // Count if any comma split card names exist more than once
  ...
  // Map card names to comma split card names if they only exist once
  ...
  const zonesToDescriptions: Record<string, string> = cardsAndTemplates.reduce(
  ...
  const sortedZoneKeys = Object.keys(zonesToDescriptions).sort((a, b) =>
    a.length == b.length
      ? a
          .split('')
  ...
  for (const zoneKey of sortedZoneKeys) {
    ...
    const reverseCardStateMap: Record<string, Card[]> = zoneCards.reduce(
      (acc: Record<string, Card[]>, card) => {
  ...
  for (const zoneGroup of zoneGroups.sort((a, b) => a.cards.length - b.cards.length)) {
    ...
    if (index === zoneGroups.length - 1 && zoneGroup.cards.length > 2) {
      ...
      const thisGroupIsOfPermanents = zoneGroup.cards.every(
        (c) => !NONPERMANENT_TYPES.some((t) => c.type.includes(t)),
      );
...
/**
 * Count the number of prerequisites in a variant.
 * @param variant - The combo variant
 * @returns Number of prerequisites
 */
export function countPrerequisites(variant: Variant): number {
