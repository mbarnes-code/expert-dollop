/**
 * Parser for identifying which built-in variables are accessed in Code Node code.
 */
...
/**
 * Wraps a function call in a Result type.
 */
function toResult<T>(fn: () => T): Result<T, Error> {
  try {
...
/**
 * Class for parsing Code Node code to identify which built-in variables are accessed.
 * This enables optimization by only fetching the data that's actually needed.
 */
export class BuiltInsParser {
   ...
   * @returns Result containing the parser state or an error
   */
   ...
   */
  private identifyBuiltInsByWalkingAst(ast: Program): BuiltInsParserState {
  ...
  /**
   * Handles $items(...) call expressions.
   */
