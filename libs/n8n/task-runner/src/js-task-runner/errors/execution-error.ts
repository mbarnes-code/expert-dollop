...
/**
 * Error that wraps execution failures in user-provided code.
 * Provides enhanced error information including line numbers.
 */
export class ExecutionError extends SerializableError {
  ...
  /** Execution context for the error */
  context: { itemIndex: number } | undefined = undefined;

  /** Line number in the user code where the error occurred */
  ...
  constructor(error: ErrorLike, itemIndex?: number) {
    ...
    // Override the stack trace with the given error's stack trace.
    // Since node v22 it's not writable, so we can't assign it directly
