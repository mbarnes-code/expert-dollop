...
/**
 * Read environment variable, with support for _FILE suffix for secrets
 */
...
/**
 * Class decorator that marks a class as a configuration class.
 * Automatically reads values from environment variables and registers with DI container.
 * 
 * @example
 * ```typescript
 * @Config
...
/**
 * Property decorator for nested configuration classes.
 * Used when a config property is itself a config class.
 * 
 * @example
 * ```typescript
 * @Config
...
/**
 * Property decorator that binds a config property to an environment variable.
 * 
 ...
 * port: number = 5678;
 * 
 * @Env('N8N_LOG_LEVEL', z.enum(['debug', 'info', 'warn', 'error']))
